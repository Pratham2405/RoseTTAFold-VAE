...
class Predictor():
    ...
    def predict(self, a3m_fn, out_prefix, hhr_fn=None, atab_fn=None, 
            custom_t2d_fn=None, window=150, shift=50):

        msa = parse_a3m(a3m_fn)
        N, L = msa.shape
        #
        if custom_t2d_fn is not None:
            # Load custom t2d from file
            t2d_data = np.load(custom_t2d_fn)  # Expects shape (1, n_templates, L, L, 64)
            t2d = torch.tensor(t2d_data).float()
            
            # Create dummy confidence scores (0.5)
            n_templates = t2d.shape[1]
            t0d = torch.ones((1, n_templates, 3)).float() * 0.5
            t1d = torch.ones((1, n_templates, L, 3)).float() * 0.5
            xyz_t = torch.full((1, n_templates, L, 3, 3), np.nan).float()
            
        elif hhr_fn != None:
            xyz_t, t1d, t0d = read_templates(L, ffdb, hhr_fn, atab_fn, n_templ=25)
            t2d = xyz_to_t2d(xyz_t, t0d)  # Compute t2d from xyz
        else:
            xyz_t = torch.full((1, 1, L, 3, 3), np.nan).float()
            t1d = torch.ones((1, 1, L, 3)).float() * 0.5
            t0d = torch.ones((1, 1, 3)).float() * 0.5
            t2d = torch.zeros((1, 1, L, L, 64)).float()
...
def get_args():
    ...
    parser.add_argument('--custom-t2d', default=None, 
                   help='Path to custom 2D template features file (numpy array)')
...
if __name__ == "__main__":
    ...
    if not os.path.exists("%s.npz"%args.out_prefix):
        ...
        pred.predict(args.a3m_fn, args.out_prefix, args.hhr, args.atab, args.custom_t2d)


# python predict_pyRosetta.py -i query.a3m -o output --custom-t2d my_t2d_features.npy
# The custom my_t2d_features.npy should be a numpy array with shape (1, n_templates, L, L, 64) 
# where L is sequence length and 64 is the template embedding dimension.
